(function ($) {
	...
	var WidgetMorphingLayoutHandler = function($scope, $) {
		...
		if(wid_sec.length){
			...
			//sections Morph
			if($scope.closest('section.elementor-element').find('> [data-morph-id="'+morph_id+'"]').length > 0){
				var remove_morph=$scope.closest('section.elementor-element').find("> .plus-morphing-svg-wrapper");
				remove_morph.remove();
			}
			//column Morph
		...
		if(morph_row=='yes' && morph_row!=undefined){
			morphing_wrap.closest('section.elementor-element').prepend(morphing_wrap);
		}
