...
function theplus_ajax_register_user( $email='', $first_name='', $last_name='',$tp_user_reg_role='' ) {
	    $errors = new \WP_Error();
		$result    = '';
	    if ( ! is_email( $email ) ) {
...
function plus_mailchimp_subscribe_using_lr($email='', $first_name='', $last_name='',$dis_mail_chimp='',$sep_cust_mail_chimp_apikey='',$sep_cust_mail_chimp_listid=''){
	...
	$result = json_decode( theplus_mailchimp_subscriber_message($email, $mc_r_status, $list_id, $api_key, array('FNAME' => $first_name,'LNAME' => $last_name) ) );	
}
...
function theplus_google_ajax_register() {
	...
	if ( isset( $_POST['email'] ) ) {
		...
		$tp_user_reg_role      = isset( $_POST['tp_user_reg_role'] ) ? $_POST['tp_user_reg_role'] : '';
		...
		if ( ! empty( $user_data ) && false !== $user_data ) {
			...
			$result  = theplus_ajax_register_user( $email, $name, $tp_user_reg_role );
...
function plus_mailchimp_subscribe(){
	...
	$result = json_decode( theplus_mailchimp_subscriber_message($_POST['email'], $mc_status, $list_id, $api_key, $chimp_field ) );
