	 ...
	 protected function render() {
		...
		if(!empty($settings["title"])){
			if ((!empty($settings['url_link']['url'])) && ($full_infobox_switch !='yes')){
				...
				$service_title= '<'.$title_tag.' class="service-title "> '.esc_html($settings["title"]).' </'.$title_tag.'>';
			}
		}
				...
				foreach($settings["loop_content"] as $item) {
						...
						if(!empty($item['loop_title'])){
							...
							if ((!empty($item['loop_url_link']['url'])) && ($item['r_full_infobox_switch'] !='yes')){
								$list_title = '<a '.$this->get_render_attribute_string( "loop_box_link".$index ).'><'.$loop_title_tag.' class="service-title ">'.esc_html($loop_title).'</'.$loop_title_tag.'></a>';						
							}else{
								$list_title = '<'.$loop_title_tag.' class="service-title">'.esc_html($loop_title).'</'.$loop_title_tag.'>';
							}							
						}
