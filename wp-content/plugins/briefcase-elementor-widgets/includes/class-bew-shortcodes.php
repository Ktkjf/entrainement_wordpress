...
class Bew_Shortcodes {
	...
	public static function init() {
		...
		foreach ( $shortcodes as $shortcode ) {
			add_shortcode( 'bew_' . $shortcode, array( __CLASS__, $shortcode ) );
		}

		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'enqueue_scripts' ) );
		add_filter( 'post_class', array( __CLASS__, 'product_class' ), 10, 3 );

		add_action( 'wp_ajax_nopriv_bew_load_products', array( __CLASS__, 'ajax_load_products' ) );
		add_action( 'wp_ajax_bew_load_products', array( __CLASS__, 'ajax_load_products' ) );
	}
	...
	public static function ajax_load_products() {
		...
		$data = self::product_loop( $atts, $template_id );
	...
	public static function product_grid( $atts ) {
		...
		return sprintf(
			'<div class="bew-product-grid bew-products %s">%s</div>',
		...
		if ( 'category' == $atts['filter'] ) {
			...
			if ( $categories && ! is_wp_error( $categories ) ) {
				if ( 'isotope' == $atts['filter_type'] ) {
					$filter[] = '<li data-filter="*" class="line-hover active">' . esc_html__( 'Show All', 'bew' ) . '</li>';
				} else {
			...
			if ( 'isotope' == $atts['filter_type'] ) {
				$filter[] = '<li data-filter="*" class="line-hover active">' . esc_html__( 'Best Sellers', 'bew' ) . '</li>';
			} else {
				$filter[] = '<li data-filter=".best_sellers" class="line-hover active">' . esc_html__( 'Best Sellers', 'bew' ) . '</li>';
			}
			$filter[] = '<li data-filter=".new" class="line-hover">' . esc_html__( 'New Products', 'bew' ) . '</li>';
			$filter[] = '<li data-filter=".sale" class="line-hover">' . esc_html__( 'Sales Products', 'bew' ) . '</li>';
		}
	...
	public static function post_grid( $atts ) {
		...
		while ( $posts->have_posts() ) : $posts->the_post();
			...
			if ( has_post_thumbnail() ) :
				...
				$thumbnail = sprintf(
					...
					get_the_post_thumbnail( get_the_ID(), 'bew-blog-grid' ),
					$icon
				);
			endif;
		...
		return sprintf(
			'<div class="bew-post-grid post-grid %s">
				<div class="row">%s</div>
			</div>',
			esc_attr( implode( ' ', $css_class ) ),
			implode( '', $output )
	...
	public static function countdown( $atts ) {
		...
		$output[] = sprintf( '<div class="timer-day box"><span class="time day"></span><span class="title">%s</span></div>', esc_html__( 'Days', 'bew' ) );
		$output[] = sprintf( '<div class="timer-hour box"><span class="time hour"></span><span class="title">%s</span></div>', esc_html__( 'Hours', 'bew' ) );
		$output[] = sprintf( '<div class="timer-min box"><span class="time min"></span><span class="title">%s</span></div>', esc_html__( 'Mins', 'bew' ) );
		$output[] = sprintf( '<div class="timer-secs box"><span class="time secs"></span><span class="title">%s</span></div>', esc_html__( 'Sec', 'bew' ) );
		$output[] = '</div>';
	...
	public static function button( $atts ) {
		...
		if ( 'center' == $atts['align'] ) {
			return '<div class="bew-button-wrapper text-center">' . $button . '</div>';
		}
			...
			'<div class="%s">
				<div class="banner-inner">
					...
					<div class="banner-content">
						...
						<a href="%s" target="%s" rel="%s" class="bew-button button-light line-hover active">%s</a>
					</div>
				</div>
			</div>',
			esc_attr( implode( ' ', $css_class ) ),
	...
	public static function product( $atts, $content ) {
		...
		return sprintf(
			...
			$content,
			$price,
			$link['title'] ? esc_html( $link['title'] ) : esc_html__( 'Add to cart', 'bew' ),
			esc_url( $link['url'] ),
			esc_attr( $link['target'] ),
			esc_attr( $link['rel'] ),
			esc_html__( 'View Product', 'bew' )
		);
	}
			...
			'<div class="%s">
				<%s>
					...
					<span class="banner-content">
						...
						<span class="bew-button button-light line-hover active">%s</span>
					</span>
				</%s>
			</div>',
			esc_attr( implode( ' ', $css_class ) ),
	...
	public static function banner_grid_4( $atts, $content ) {
		...
		$css_class = array( 'bew-banner-grid-4', $atts['el_class'] );
	...
	public static function banner_grid_5( $atts, $content ) {
		...
		$css_class = array( 'bew-banner-grid-5', $atts['el_class'] );
	...
	public static function banner_grid_6( $atts, $content ) {
		...
		$css_class = array( 'bew-banner-grid-6', $atts['el_class'] );
	...
	public static function icon_box( $atts, $content ) {
		$atts = shortcode_atts( array(
			...
			'image'            => '',
			'style'            => 'normal',
			'title'            => esc_html__( 'I am Icon Box', 'bew' ),
			'css_animation'    => '',
			'el_class'         => '',
		), $atts, 'bew_' . __FUNCTION__ );
	...
	public static function pricing_table( $atts ) {
		$atts = shortcode_atts( array(
			...
			'price'         => '',
			'currency'      => '$',
			'recurrence'    => esc_html__( 'Per Month', 'bew' ),
			'features'      => '',
			'button_text'   => esc_html__( 'Get Started', 'bew' ),
			'button_link'   => '',
			'color'         => '#6dcff6',
			'css_animation' => '',
			'el_class'      => '',
	...
	public static function map( $atts, $content ) {
		...
		if ( empty( $atts['api_key'] ) ) {
			return esc_html__( 'Google map requires API Key in order to work.', 'bew' );
		}

		if ( empty( $atts['address'] ) && empty( $atts['lat'] ) && empty( $atts['lng'] ) ) {
			return esc_html__( 'No address', 'bew' );
		}
	...
	public static function info_list( $atts ) {
		$atts = shortcode_atts( array(
			'info' => urlencode( json_encode( array(
				array(
					'icon' => 'fa fa-home',
					'label' => esc_html__( 'Address', 'bew' ),
					'value' => '9606 North MoPac Expressway',
				),
				array(
					'icon' => 'fa fa-phone',
					'label' => esc_html__( 'Phone', 'bew' ),
					'value' => '+1 248-785-8545',
				),
				array(
					'icon' => 'fa fa-fax',
					'label' => esc_html__( 'Fax', 'bew' ),
					'value' => '123123123',
				),
				array(
					'icon' => 'fa fa-envelope',
					'label' => esc_html__( 'Email', 'bew' ),
					'value' => 'bew@uix.store',
				),
				array(
					'icon' => 'fa fa-globe',
					'label' => esc_html__( 'Website', 'bew' ),
	...
	public static function faq( $atts, $content ) {
		$atts = shortcode_atts( array(
			'title'         => esc_html__( 'Question content goes here', 'bew' ),
			'open'          => 'false',
			'css_animation' => '',
			'el_class'      => '',
		), $atts, 'bew_' . __FUNCTION__ );
		...
		return sprintf(
			...
			esc_attr( implode( ' ', $css_class ) ),
			esc_html__( 'Question', 'bew' ),
			esc_html( $atts['title'] ),
			esc_html__( 'Answer', 'bew' ),
			$content
		);
	}
	...
	public static function get_coordinates( $address, $key = '', $refresh = false ) {
		...
		if ( $refresh || $coordinates === false ) {
			...
			if ( is_wp_error( $response ) ) {
				$results['error'] = esc_html__( 'Can not connect to Google Maps APIs', 'bew' );
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
					...
					$results['error'] = esc_html__( 'No location found for the entered address.', 'bew' );
				} elseif ( $data->status === 'INVALID_REQUEST' ) {
					$results['error'] = esc_html__( 'Invalid request. Did you enter an address?', 'bew' );
				} else {
					$results['error'] = $data->error_message;
				}
			} else {
				$results['error'] = esc_html__( 'Unable to contact Google API service.', 'bew' );
			}
		} else {
	 ...
	 */
	protected static function product_loop( $atts, $template_id,  $loop_name = 'bew_product_grid' ) {
		global $woocommerce_loop;
		...
		// Bew Products Div										
		if ( 'vertical' == $settings['shop_filter_type']) {
			?>
			<div class="bew-products hide-left">								
			<?php	
		}else {
			?>
		...
		if ( $products->have_posts() ) {
			...
			while ( $products->have_posts() ) : $products->the_post();
					 ...
					 if($row == 1) {
						...
						$data = apply_filters( 'elementor/frontend/builder_content_data', $data, $template_id );
