...
class Bew_Categories_Block_1 {
  ...
  public function controls( $widget ) {
	...
	Bew_Controls::add_font_size( $widget, $id, [ 'default' => 19, 'default_unit' => 'px' , 'selectors' => 'h5' ] );
      ...
      <?php $widget->html('section_header'); ?>
        ...
        <div class="bew-row gap-y gap-2">
          ...
          foreach ( $settings['t1_items'] as $item ) :
            ...
            $link  = $item['link'];
			$count = '<mark class="count">' .$item['count'] .'</mark>';
            if ( '0' !== $item['cat'] ) {
              $id = intval( $item['cat'] );              
			  ...
			  $count = sprintf( // WPCS: XSS OK.
				/* translators: 1: number of products */
				_nx( '<mark class="count">%1$s Product</mark>', '<mark class="count">%1$s Products</mark>', $category->count, 'product categories', 'briefcasewp-extras' ),
          ...
          ?>
             ...
             <div class="cat-block <?php echo $col_class; ?>">
              ...
              <a class="categories-1" href="<?php echo esc_url( $link ); ?>">
